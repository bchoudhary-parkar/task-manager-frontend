...
const TaskModal: React.FC<TaskModalProps> = ({ task, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (task) {
      ...
      setFormData({
        ...
        assignedTo: task.assignedTo, // This should be the _id from the task
  ...
  const fetchUsers = async () => {
    try {
      ...
      const data = await taskApi.getUsersForAssignment({ page: 1, limit: 20 }); 
      setUsers(data);
    } catch (error: any) {
  ...
  const validateField = (name: string, value: string): string | undefined => {
    if (name === 'title') {
      ...
      if (!value) return 'Please select a user to assign this task';
    } else if (name === 'priority') {
      if (!value) return 'Please set your priority for this task';
    } else if (name === 'dueDate') {
      if (!value) return 'Due Date is required for this task';
    }
    return undefined;
  };
